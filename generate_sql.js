...
let sql = 'INSERT INTO products (name, price, image_url, category, description, material, size) VALUES\n';
const values = allProducts.map(p => {
  return `('${p.name}', ${p.price}, '${p.image_url}', '${p.category}', '${p.description.replace(/'/g, "''")}', '${p.material}', '${p.size}')`;
}).join(',\n');
fs.writeFileSync('insert_products.sql', sql + values + ';');
